...
public_fun:
    ColorShaderCls();
    ColorShaderCls(const ColorShaderCls &);
    ~ColorShaderCls();

    bool fInitialize(ID3D11Device *, HWND);
    void fShutdown();
    bool fRender(ID3D11DeviceContext *, int, D3DXMATRIX, D3DXMATRIX, D3DXMATRIX);
...
private_fun:
    ...
    bool fSetShaderParameters(ID3D11DeviceContext *, D3DXMATRIX, D3DXMATRIX, D3DXMATRIX);
